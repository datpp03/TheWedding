...
| `storage`       | `StorageService` adapter, local storage, controller serve file                                                                                                                                            | Local only; R2/S3 adapter & signed URL chưa có                                                                            |
| `scale`         | Phase 9 SaaS scale foundation: B2C/B2B plan catalog, add-ons, entitlements, user handles, tenant quota summaries, analytics events, payment-event idempotency, custom-domain/studio/greeting placeholders | Foundation done; real MoMo checkout/webhook, R2 adapter, direct upload sessions, canonical public handle routes chưa xong |
