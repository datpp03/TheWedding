...
1. Cap nhat `VIEC_CAN_LAM.md` (thu muc goc): liet ke viec NGUOI DUNG can lam (credentials, config, dich vu tra phi, QA thu cong, quyet dinh san pham) va acceptance chua hoan thanh. Moi item phai kem huong dan thuc hien chi tiet ngay ben duoi: can chuan bi gi, cac buoc lam, file/env/dashboard/URL lien quan, cach smoke test/xac nhan pass, va link docs lien quan.
2. Neu con loi/viec khan cap, chen vao `## Carryover Khan Cap Tu Prompt Truoc` o dau prompt KE TIEP theo thu tu khuyen nghi ben duoi; neu da la prompt cuoi thi ghi vao muc "Khan cap" cua `VIEC_CAN_LAM.md`.
3. Ghi y tuong nang cap/mo rong tu nghi ra vao `Y_TUONG_NANG_CAP.md` (thu muc goc).
...
- Khi ghi `VIEC_CAN_LAM.md`, khong ghi task ngan chung chung. Moi task phai co mini-runbook bang tieng Viet: muc tieu, prerequisites/credentials, cac buoc thao tac, noi can cau hinh/kiem tra, expected result, va docs lien quan.
